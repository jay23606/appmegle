...
(function () {
    ...
    let particles = [], nearText = '', nearTimer = 0, shake = 0;
    ...
    const begin = (sd, n, tally) => {
        ...
        hit = new Set(); collected = new Set(); passed = new Set(); particles = []; nearText = ''; nearTimer = shake = 0; winner = null; opp = { d: 0, l: 1, j: 0, boost: 0, crash: 0 };
    ...
    const burst = (x, y, n, colors) => { for (let i=0;i<n;i++) particles.push({x,y,vx:(Math.random()-.5)*330,vy:-70-Math.random()*260,life:.55+Math.random()*.8,col:colors[i%colors.length],size:2+Math.random()*5}); };
    ...
    const step = (dt) => {
        ...
        for (let i = 0; i < course.length; i++) {
            ...
            if (dz < -25 && !passed.has(i)) {
                passed.add(i);
                if (!hit.has(i) && !collected.has(i) && o.type !== 'coin' && o.type !== 'boost' && Math.abs(laneX-o.lane)<.58) { nearText='NEAR MISS!';nearTimer=.65; }
            }
            ...
            if (!safe) { hit.add(i); crash = .75; boost = 0; shake=.35; burst(project(0,laneX).x,GROUND-30,25,['#ff385c','#ffb347','#fff']); ctx.send({ t: 'crash' }); }
        }
    ...
    const draw = () => {
        ...
        if(boost>0){g.strokeStyle='#ffffff80';g.lineWidth=2;for(let i=0;i<18;i++){const x=(i*97+t*430)%W,y=85+(i*47%270);g.beginPath();g.moveTo(x,y);g.lineTo(x-35-Math.random()*45,y);g.stroke();}}
        if (opp.d > dist - 60 && opp.d < dist + VIEW) { const q = project(opp.d-dist, opp.l); runner(q.x, q.y-opp.j*q.p, .25+q.p*.75, me==='a'?'#ff9d3d':'#5db4ff', false, opp.boost?'#ffe45e':null,dist/24); }
        const myX = project(0,laneX).x; runner(myX, GROUND-jumpY, 1, me==='a'?'#5db4ff':'#ff9d3d', sliding>0, boost>0?'#ffe45e':null,dist/20);
        ...
        if (phase==='count') { const left=Math.max(0,countEnd-performance.now()),n=Math.max(0,Math.ceil((left-200)/1000));g.fillStyle='#0005';g.fillRect(0,0,W,H);g.fillStyle='#fff'; g.textAlign='center'; g.font='900 68px system-ui'; g.fillText(n>0?n:'GO!',W/2,H/2+20); }
    ...
    const loop = (t) => { const dt=Math.min(.035,(t-lastT)/1000||0); lastT=t; if(phase==='count'&&performance.now()>=countEnd)phase='run'; step(dt);stepFx(dt); if (phase==='run' && t-lastSend>80) { lastSend=t; ctx.send({t:'p',d:Math.round(dist),l:+laneX.toFixed(2),j:Math.round(jumpY),boost:boost>0,crash:crash>0}); } draw(); status(); raf=requestAnimationFrame(loop); };
